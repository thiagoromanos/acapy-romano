...
class RevocationManager:
    ...
    async def unrevoke_credential_by_cred_ex_id(
        self,
        cred_ex_id: str,
        publish: bool = False,
        notify: bool = False,
        ...
        """UnRevoke a credential by its credential exchange identifier at issue.
        ...
        return await self.unrevoke_credential(
            rev_reg_id=rec.rev_reg_id,
            cred_rev_id=rec.cred_rev_id,
            publish=publish,
            notify=notify,
        ...
        if notify:
            ...
            async with self._profile.session() as session:
                await rev_notify_rec.save(
                    session, reason="New unrevocation notification"
                )

        if publish:
            await revoc.get_or_fetch_local_tails_path(rev_reg_def)
            result = await revoc.unrevoke_pending_credentials(
                rev_reg_id,
                additional_crids=[int(cred_rev_id)],
            )

            if result.curr and result.revoked:
                await self.set_cred_unrevoked_state(rev_reg_id, result.revoked)
                await revoc.update_revocation_list(
                    rev_reg_id,
                    result.prev,
                    result.curr,
    ...
    async def set_cred_unrevoked_state(
        self, rev_reg_id: str, cred_rev_ids: Sequence[int]
    ) -> None:
        """Update credentials state to credential_revoked.
        ...
        for cred_rev_id in cred_rev_ids:
            ...
            try:
                async with self._profile.transaction() as txn:
                    ...
                    cred_ex_id = rev_rec.cred_ex_id
                    cred_ex_version = rev_rec.cred_ex_version
                    rev_rec.state = IssuerCredRevRecord.STATE_ISSUED
                    await rev_rec.save(txn, reason="revoke credential")
                    await txn.commit()
            except StorageNotFoundError:
                continue
